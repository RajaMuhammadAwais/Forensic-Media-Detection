...
class VideoDetector:
    def __init__(self, model_type='cnn_lstm', use_wavelet_preprocessor=False):
        ...
        self.model = None
        self.use_wavelet_preprocessor = use_wavelet_preprocessor
        if self.use_wavelet_preprocessor:
    ...
    def extract_frames(self, video_path, num_frames=30, target_size=(256, 256)):
        ...
        # Apply wavelet preprocessing if enabled
        if self.preprocessor:
            processed_frames = []
            for frame in frames:
                # WaveletPreprocessor expects 0-255 range, so we don't normalize yet
                processed_frame = self.preprocessor.process_frame(frame.astype(np.uint8))
                processed_frames.append(processed_frame)
            frames = np.array(processed_frames)
            
            # Normalize the wavelet coefficients
            # The normalization range depends on the model's input requirements.
            # For simplicity, we normalize to [0, 1] based on the max absolute value.
            max_val = np.max(np.abs(frames))
            if max_val > 0:
                frames = frames / max_val
            
        else:
            # Normalize pixel values to [0, 1] for standard processing
            frames = frames / 255.0
