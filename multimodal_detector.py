...
from .audio_visual_verifier import AudioVisualVerifier
...
class MultimodalDetector:
    def __init__(self, fusion_type="late"):
        ...
        self.av_verifier = AudioVisualVerifier() # New verifier for audio-visual consistency
        ...
        if file_ext in [".mp4", ".avi", ".mov", ".mkv"]:
            try:
                ...
                if video_frames.size > 0:
	                    ...
	                    # Use the new AudioVisualVerifier for lip-sync check
	                    # NOTE: This assumes the video file contains both video and audio streams.
	                    # A more robust implementation would require extracting the audio stream first.
	                    # For this placeholder, we assume the media_path is sufficient.
	                    av_consistency_result = self.av_verifier.verify_consistency(media_path, media_path)
	                    
	                    results["video_analysis"] = {
	                        "deepfake_probability": float(video_pred),
	                        "audio_visual_consistency": av_consistency_result
	                    }
                else:
